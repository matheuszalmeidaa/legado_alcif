-- ============================================================
-- View: qlik_legado_c6
-- Banco C6 (id_banco = 129), de 2022-01-01 até hoje, prazos
-- 84/96/108, unindo:
--   - histórico: csd.python.qlik
...
WHERE TRIM(lf.id_proposta) SIMILAR TO '[0-9]+(\.[0-9]+)?'
    ...
    AND TO_DATE(SUBSTR(lf.dat_crc, 1, 10), 'yyyy-MM-dd') >= DATE '2022-01-01'
...
WHERE p.id_banco = 129
    ...
    AND TO_DATE(p.dat_crc, 'dd/MM/yyyy HH:mm:ss') >= DATE '2022-01-01'
    AND NOT EXISTS (
        SELECT 1
